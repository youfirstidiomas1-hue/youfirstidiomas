document.addEventListener('DOMContentLoaded', () => {
    ...
    if (loginForm) {
        loginForm.addEventListener('submit', async (e) => {
            ...
            // Reemplaza esta URL con tu enlace de Google Sheets API (ej. SheetBest / AppScript)
            const SHEET_API_URL = "TU_URL_DE_API_GOOGLE_SHEETS"; 

            try {
                // Petición al Excel/Google Sheets
                const response = await fetch(SHEET_API_URL);
                const students = await response.json();

                // Buscar coincidencia de email
                const student = students.find(s => s.Email.toLowerCase() === emailInput);
